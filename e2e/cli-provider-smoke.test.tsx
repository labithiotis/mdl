...
describe('cli-provider-smoke', () => {
  for (const [provider, url] of Object.entries(ALBUM_EXAMPLES)) {
    test(
      `downloads the first track from the album for ${provider}`,
      async () => {
  ...
  for (const [provider, url] of Object.entries(TRACK_EXAMPLES)) {
    test(
      `downloads the track for ${provider}`,
      async () => {
...
async function runProviderSmokeTest(
  ...
  const outputRoot = await mkdtemp(path.join(os.tmpdir(), `mdl-e2e-${provider}-`));
  const app = render(
    <App
      audioFormat="mp3"
      audioQuality="best"
