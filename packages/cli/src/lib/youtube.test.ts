...
describe('youtube client fallback detection', () => {
  ...
  test('recognizes retryable client fallback errors', () => {
    ...
    expect(
      isRetryableYouTubeClientError(new Error('No valid URL to decipher'))
    ).toBe(true);
    expect(
      isRetryableYouTubeClientError(
    ...
    expect(
      isRetryableYouTubeClientError(
        new Error('The server responded with a non 2xx status code')
      )
    ).toBe(true);
    expect(
      isRetryableYouTubeClientError(
        new Error('YouTube audio stream request failed with status 403.')
      )
    ).toBe(true);
  });
});
